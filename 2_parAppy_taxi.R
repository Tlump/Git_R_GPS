...
clusterEvalQ(clus, calPeriod_car <- function(CarID,Date){
  select <- taxi[taxi$CarID==CarID & taxi$Date==Date,]
...
clusterExport(clus,"taxi")


taxi.Period <- do.call(rbind,parRapply(clus,taxi.count[,c("CarID","Date")],function(row) calPeriod_car(row["CarID"],row["Date"])))

# Finish
stopCluster(clus)

write.csv(taxi.Period, file = "/Users/TLump_MAC/Thesis/Data/temp/TaxiPeriod4.csv")
